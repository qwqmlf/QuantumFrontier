...
const useStyles = makeStyles(theme => ({
  ...
  itemStyle: {
    ...
    fontFamily:
      "Quicksand YuGothic HiraginoKakuGothic Meiryo Osaka MSPGothic sansSerif"
  },
  imgStyle: {
...
function Qi_for_beginners1() {
  ...
  return (
    <div>
      <div className={classes.title}>量子コンピュータ基礎１</div>
      <div className={classes.paragraph}>
        <p>
          量子コンピュータ、それは果たしてどのようなコンピュータなのでしょうか。
        </p>
        <p>
          計算機について語る上で、理想的な計算機(コンピュータ)のモデルとして定義される
          <a href="https://ja.wikipedia.org/wiki/%E3%83%81%E3%83%A5%E3%83%BC%E3%83%AA%E3%83%B3%E3%82%B0%E3%83%9E%E3%82%B7%E3%83%B3">
            チューリングマシン
          </a>
          があります。
          ある計算機とこのチューリングマシンとの比較をすることで、その計算機(今回は量子コンピュータ)がどの程度のものなのかを知ることができます。
          量子計算機のモデルの１つである量子チューリングマシンは、理想的な計算機であるチューリングマシンと全く同じ
          <a href="https://ja.wikipedia.org/wiki/%E8%A8%88%E7%AE%97%E5%8F%AF%E8%83%BD%E6%80%A7%E7%90%86%E8%AB%96">
            計算可能性
          </a>
          (どんな問題を解くことができるか)をもつとされます。
        ...
        <p>
          ...
          量子ゲート方式は既存のコンピュータの汎用性を引き継ぎ、
          さらに計算を効率化することを目標としています。 それに対し、
          量子アニーリングは多体系を用いた系を使って問題を解こうとするものです。
          このページでは量子ゲート方式の方を量子計算機と呼称します。
        </p>

        <p>
          現在世界では量子計算機開発の競争は日に日に拡大しています。世界中の研究室で量子計算機開発は続いており、表のような様々な物質の性質を用いた量子計算が研究されています。
        </p>
      ...
      <div className={classes.stitle}>量子力学的性質</div>
      <div className={classes.paragraph}>
        <p>
      ...
      <div className={classes.stitle}>量子ビット</div>
      <div className={classes.paragraph}>
        <p>
          古典コンピュータでは、情報の最小単位としてビット(bit)を用います。
          ビットは、0もしくは1の2通りの情報を表現することができます。 これを
          <InlineMath math="n" />
          個用意することによって、
          <InlineMath math="2^n" />
          の状態を表現することができます。
        ...
        <p>
          ...
          (プサイ)や
          <InlineMath math="\phi" />
          (ファイ)を用います。 なお、複数の量子の状態を表す時、
          <InlineMath math="\vert  \rangle" />
          の中に複数の状態を入れることもできます。 例えば、
      ...
      <div className={classes.stitle}>重ね合わせ</div>
      <div className={classes.paragraph}>
        <p>
          ...
          <BlockMath
            math="\vert \psi\rangle =a\vert  0\rangle+b\vert  1\rangle = \bigg(
                    \begin{array}{r}
                    a  \\
                    b   \\
                    \end{array}
          ...
          で状態
          <InlineMath math="\vert  0\rangle" />、 確率
          <InlineMath math="\vert  b\vert  ^2" />
          で状態
          <InlineMath math="\vert  1\rangle" />
          が観測できるという状態です。 例えば、
          <BlockMath
            math="\vert  \psi\rangle = \frac{\vert 0\rangle+\vert  1\rangle}{\sqrt{2}}= \frac{1}{\sqrt{2}} 
                    {\bigg(
                    \begin{array}{r}
                    1  \\
                    1   \\
      ...
      <div className={classes.stitle}>量子もつれ</div>
      <div className={classes.paragraph}>
        <p>
      ...
      <div className={classes.paragraph}>
        次のページ：
        <a href="https://qwqmlf.github.io/QuantumFrontier/article/qi_for_beginners2">
          量子コンピュータ基礎２
        </a>
        では、量子回路を中心に量子計算について解説していきます。
      </div>
    </div>
  );
}
